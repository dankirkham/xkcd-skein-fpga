...
class SkeinGenerator():
    ...
    def initialize_key(self, key_ptr, constant_ptr):
        ...
        Attributes:
        key_ptr -- Pointer to the first word in the key. Where the key should
        be stored in RAM.
        ...
        for (index, value) in enumerate(SkeinGenerator.key):
            self.f.write("// CoreSimInput {} // Key Word: {}\n".format(value,
                                                                       index))
            self.f.write("Constant {}\n".format(constant_ptr + index))
            self.f.write("Save {}\n".format(key_ptr + index))
    ...
    def initialize_tweak(self, tweak_ptr, constant_ptr):
        ...
        Attributes:
        tweak_ptr -- Pointer to the first tweak value. This is where the tweak
        will be stored in RAM.
        constant_ptr -- Pointer to the tweak on the Constants ROM. This where
    ...
    def calculate_tweak_extend(self, tweak_ptr):
        ...
        other tweak words are XORed together. The result is stored at tweak_ptr
        + 2.
